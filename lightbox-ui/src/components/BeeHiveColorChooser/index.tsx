...
import './style.css';
import { HIVE_IMG } from "./data";

interface IBeeHiveColorChooser {
    onInput: (color: any) => void;
...
const BeeHiveColorChooser: Component<IBeeHiveColorChooser> = (props: IBeeHiveColorChooser) => {

    const clickColor = (color: string, x: number, y: number) => {
    ...
    return <div class="beehive-container">
        <h2>Choose a colour:</h2>
        <div class="honey-comb">
            <img class="colorpicker" style="margin-right:2px;" src={`data:image/png;base64,${HIVE_IMG}`} usemap="#colorMap" alt="colorMap" />
            <map id="colorMap" name="colormap" onMouseOut={() => { }}>
        ...
        <span class="note">*Click on a colour to send to the Lightbox.</span>
    </div>
}

export default BeeHiveColorChooser;